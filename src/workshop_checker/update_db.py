...
def fetch_workshop_pages(itemIds: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    raw_data: Dict[str, Any] = {"itemcount": len(itemIds)}
    currentItemCount = 0
    ...
    if result_count != len(itemIds):
        ...
        raise ValueError(
            f"The API returned {result_count} results, but we "
            f"expected {len(itemIds)}!"
        )
    result = json_data["response"]["result"]
    if result != 1:
        raise ValueError(
            f'The API returned result "{result}", but we expected "1"!'
        )
    ...
    for result in json_data["response"]["publishedfiledetails"]:
        ...
        if mod_id not in itemIds:
            raise ValueError(
                f"The API returned a result for an item with "
...
def check_mod_update(
    mod_id: str,
    workshop_timestamp: int,
    local_mods: Dict[str, int],
    download_new=True,
) -> bool:
    try:
        ...
        logger.log(
            5,
            f"ID: {mod_id}, local: {local_timestamp}, "
...
def check_updates(
    ws_path: str, mods_info: Dict[str, Dict[str, Any]], download_new=True
) -> List[str]:
    results = []
    ...
    for item_id, mod_info in mods_info.items():
        if check_mod_update(
            item_id, mod_info["timestamp"], local_mods, download_new
        ):
...
def send_mail(
    message_text: str, recipients: list, subject="Zeusops Pending Mod Updates"
):
    ...
    try:
        ...
        msg["To"] = (
            ", ".join(recipients) if len(recipients) > 1 else recipients[0]
        )
...
def cli(argv: list[str] | None = None):
    ...
    if args.check_updates:
        ...
        updated_mod_ids = check_updates(
            args.workshop_path, mods_info, not args.only_existing
        )
        ...
        if updated_mod_count > 0 and args.notify:
            ...
            needs_mail = any(
                elem not in last_mailed for elem in updated_mod_ids
            )
            needs_discord = any(
                elem not in last_discord for elem in updated_mod_ids
            )
            if needs_mail or needs_discord:
                ...
                if args.notify_discord and needs_discord:
                    ...
                    webhook = DiscordWebhook(
                        url=webhook_url, content=message_text
                    )
                    webhook.execute()
